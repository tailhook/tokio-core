...
/// Multiply duration by u64
///
/// Rust only supports multiplying Duration by u32, so we have to
/// multiply it manually.
///
/// Note: this implementatin crashes on multiplier bigger than (1 << 63) - 1,
/// but we can't get a number this large in the next_interval function.
fn duration_multiply(dur: Duration, mult: u64) -> Duration {
    let high_bits = (mult >> 31) as u32;
    let low_bits = (mult & ((1 << 31) - 1)) as u32;
    return dur*high_bits*(1 << 31) + dur*low_bits;
}
...
fn next_interval(prev: Instant, now: Instant, interval: Duration) -> Instant {
    ...
    if new > now {
        ...
        return prev + duration_multiply(interval, mult);
    }
}
